...
with st.sidebar:
    ...
    commission_rate = st.number_input("佣金费率", value=0.0003, step=0.0001, format="%.4f")
    stamp_tax = st.number_input("印花税率", value=0.001, step=0.0001, format="%.4f")
    slippage = st.number_input("滑点比例", value=0.001, step=0.0005, format="%.4f")
...
if run_btn:
    ...
    if strategy_name == "双均线交叉":
        ...
        strategy = MACDStrategy(fast=macd_fast, slow=macd_slow, signal_period=macd_signal)
    else:
    ...
    # K线图 + 信号
    st.plotly_chart(plot_candlestick_with_signals(data, result), use_container_width=True)
    ...
    if result.trades:
        trades_data = [
            {
                "日期": str(t.date.date()) if hasattr(t.date, "date") else str(t.date),
